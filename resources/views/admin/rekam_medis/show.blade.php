...
<div class="max-w-5xl mx-auto py-8 space-y-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            ...
            <p class="text-xs text-gray-400 font-medium uppercase tracking-widest mt-1">Informasi lengkap kunjungan pasien</p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div class="p-8 border-b border-gray-100">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Pasien</p>
                    <p class="text-sm font-bold text-gray-800">{{ optional($rekamMedi->pasien)->name ?? 'N/A' }}</p>
                    <p class="text-[10px] text-gray-400 font-medium mt-0.5">RM: {{ optional($rekamMedi->pasien)->no_rm ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tenaga Medis (Dokter)</p>
                    <p class="text-sm font-bold text-gray-800">{{ optional($rekamMedi->dokter)->name ?? 'Dokter Pemeriksa' }}</p>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Waktu Pemeriksaan</p>
                    <p class="text-sm font-bold text-gray-800">{{ \Carbon\Carbon::parse($rekamMedi->tanggal)->format('d F Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 border-b border-gray-100 bg-gray-50/30">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-800 leading-tight">{{ $rekamMedi->diagnosis ?? '-' }}</p>
                </div>
                <div>
        ...
        <div class="p-8 border-b border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600 font-medium whitespace-pre-line">{{ $rekamMedi->pemeriksaan_lab ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Tindakan Medis</p>
                    <p class="text-sm text-gray-600 font-medium whitespace-pre-line">{{ $rekamMedi->tindakan ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 space-y-8">
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @forelse($rekamMedi->obats as $obat)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-100">
                <div>
                    ...
                    <p class="text-sm text-gray-600 font-medium">{{ $rekamMedi->rencana_tindak_lanjut ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Rujukan</p>
                    <p class="text-sm text-gray-600 font-medium">{{ $rekamMedi->rujukan ?? '-' }}</p>
                </div>
            </div>
        </div>
...
<style>
    @media print {
        aside, nav, .flex.justify-between.items-center.mb-4, .mb-4, .flex.justify-center.pt-4 { display: none !important; }
